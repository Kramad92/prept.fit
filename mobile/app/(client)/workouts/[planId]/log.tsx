      ...
      <SafeAreaView className="flex-1 bg-gray-50" edges={["top"]}>
        ...
        <ScrollView className="flex-1 px-4 pt-8" keyboardShouldPersistTaps="handled">
          <View className="items-center mb-8">
            ...
            <Text className="text-2xl font-bold text-gray-900">Workout Complete!</Text>
          </View>
          ...
          {exerciseLogs.map((ex) => {
            ...
            return (
              <View key={ex.exerciseId} className="bg-white rounded-xl p-3 mb-2 border border-gray-100 flex-row items-center">
                <Check size={16} color={done > 0 ? "#059669" : "#d1d5db"} />
                <Text className="flex-1 ml-2 text-sm text-gray-900">{ex.name}</Text>
                <Text className="text-sm text-gray-500">{done}/{ex.sets.length} sets</Text>
              </View>
            );
          })}

          <TextInput
            className="bg-white rounded-xl p-4 mt-4 border border-gray-100 text-base text-gray-900 min-h-[80px]"
    ...
    <SafeAreaView className="flex-1 bg-gray-50" edges={["top"]}>
      ...
      <View className="flex-row items-center px-4 py-3 bg-white border-b border-gray-100">
        ...
        <View className="flex-1">
          <Text className="text-sm text-gray-500">Exercise {currentIndex + 1} of {exercises.length}</Text>
      ...
      {isResting && (
        <View className="absolute top-0 left-0 right-0 bottom-0 z-50 bg-gray-900/80 items-center justify-center">
          <View className="bg-white rounded-2xl p-8 mx-8 items-center">
            ...
            <Text className="text-gray-500 mt-2">Rest Time</Text>
      ...
      <ScrollView className="flex-1 px-4 pt-4" keyboardShouldPersistTaps="handled">
        ...
        <View className="flex-row gap-3 mb-4">
          {currentExercise.sets != null && (
            <View className="bg-brand-50 rounded-lg px-3 py-1.5">
              <Text className="text-xs text-brand-700 font-medium">{currentExercise.sets} sets</Text>
            </View>
          )}
          {currentExercise.reps && (
            <View className="bg-brand-50 rounded-lg px-3 py-1.5">
              <Text className="text-xs text-brand-700 font-medium">{currentExercise.reps} reps</Text>
            </View>
          )}
        ...
        <View className="mb-2">
          <View className="flex-row mb-2 px-1">
            <Text className="text-xs text-gray-400 w-12">SET</Text>
            <Text className="text-xs text-gray-400 flex-1 text-center">REPS</Text>
            <Text className="text-xs text-gray-400 flex-1 text-center">WEIGHT</Text>
            <View className="w-11" />
          </View>
          ...
          {currentLog?.sets.map((set, idx) => (
            <View
              key={idx}
              className={`flex-row items-center rounded-xl p-3 mb-2 border ${set.completed ? "bg-brand-50 border-brand-200" : "bg-white border-gray-100"}`}
            >
              <Text className="text-sm font-bold text-gray-500 w-12">{idx + 1}</Text>
              <TextInput
                className="flex-1 text-center text-base text-gray-900 bg-gray-50 rounded-lg py-2 mx-1"
                value={set.reps}
              ...
              />
              <TouchableOpacity
                className={`w-11 h-11 rounded-full items-center justify-center ${set.completed ? "bg-brand-500" : "bg-gray-100"}`}
                onPress={() => toggleSet(idx)}
              >
                <Check size={18} color={set.completed ? "white" : "#9ca3af"} />
              </TouchableOpacity>
            </View>
          ))}
      ...
      <View className="flex-row gap-3 p-4 bg-gray-50">
        {currentIndex > 0 && (
          <TouchableOpacity className="flex-1 rounded-xl py-3.5 items-center border border-gray-200 bg-white" onPress={goPrev}>
