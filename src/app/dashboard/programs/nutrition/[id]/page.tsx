...
import type { NutritionProgramDetail } from "@/types";
...
export default function NutritionProgramDetailPage() {
  const t = useT();
  const params = useParams();
  const router = useRouter();
  const [program, setProgram] = useState<NutritionProgramDetail | null>(null);
  ...
  useEffect(() => {
    Promise.all([
      api.get<NutritionProgramDetail>(`/api/nutrition-programs/${params.id}`),
  ...
  async function handleAssign() {
    ...
    setAssigning(true);
    try {
      await api.post("/api/nutrition-programs/assign", {
        clientId: assignClientId,
        programId: params.id,
        startDate,
        accessPolicy,
      });
      const updated = await api.get<NutritionProgramDetail>(
        `/api/nutrition-programs/${params.id}`
      );
      setProgram(updated);
      setShowAssign(false);
      setAssignClientId("");
    ...
    <div>
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 flex flex-wrap gap-3 text-sm text-gray-500">
          ...
          <span>
            {program.mealsPerDay} {t.programs.mealsPerDay}
          </span>
          <span>
            {program.days.length} {t.programs.mealPlansCount}
          </span>
        </div>

        <div className="mt-3 flex flex-wrap gap-2">
          <Button variant="outline" asChild className="text-sm">
            <Link href={`/dashboard/programs/nutrition/${program.id}/edit`}>
